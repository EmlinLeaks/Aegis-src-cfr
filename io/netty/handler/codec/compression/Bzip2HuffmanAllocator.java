...
final class Bzip2HuffmanAllocator {
    private static int first(int[] array, int i, int nodesToMove) {
        ...
        i = Math.max((int)(nodesToMove - 1), (int)i);
        while (k > i + 1) {
            int temp = i + k >>> 1;
    ...
    private static int findNodesToRelocate(int[] array, int maximumLength) {
        ...
        int currentDepth = 1;
        while (currentDepth < maximumLength - 1) {
            if (currentNode <= 1) return currentNode;
            currentNode = Bzip2HuffmanAllocator.first((int[])array, (int)(currentNode - 1), (int)0);
            ++currentDepth;
        }
        return currentNode;
    }
    ...
    private static void allocateNodeLengths(int[] array) {
        ...
        while (availableNodes > 0) {
            ...
            firstNode = Bzip2HuffmanAllocator.first((int[])array, (int)(lastNode - 1), (int)0);
    ...
    private static void allocateNodeLengthsWithRelocation(int[] array, int nodesToMove, int insertDepth) {
        ...
        while (availableNodes > 0) {
            int lastNode = firstNode;
            firstNode = firstNode <= nodesToMove ? firstNode : Bzip2HuffmanAllocator.first((int[])array, (int)(lastNode - 1), (int)nodesToMove);
            int offset = 0;
            if (currentDepth >= insertDepth) {
                offset = Math.min((int)nodesLeftToMove, (int)(1 << currentDepth - insertDepth));
    ...
    static void allocateHuffmanCodeLengths(int[] array, int maximumLength) {
        ...
        Bzip2HuffmanAllocator.setExtendedParentPointers((int[])array);
        int nodesToRelocate = Bzip2HuffmanAllocator.findNodesToRelocate((int[])array, (int)maximumLength);
        if (array[0] % array.length >= nodesToRelocate) {
            Bzip2HuffmanAllocator.allocateNodeLengths((int[])array);
            return;
        }
        int insertDepth = maximumLength - (32 - Integer.numberOfLeadingZeros((int)(nodesToRelocate - 1)));
        Bzip2HuffmanAllocator.allocateNodeLengthsWithRelocation((int[])array, (int)nodesToRelocate, (int)insertDepth);
    }
