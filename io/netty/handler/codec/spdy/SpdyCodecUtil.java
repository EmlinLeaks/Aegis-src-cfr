...
final class SpdyCodecUtil {
    ...
    static int getUnsignedShort(ByteBuf buf, int offset) {
        return (buf.getByte((int)offset) & 255) << 8 | buf.getByte((int)(offset + 1)) & 255;
    }

    static int getUnsignedMedium(ByteBuf buf, int offset) {
        return (buf.getByte((int)offset) & 255) << 16 | (buf.getByte((int)(offset + 1)) & 255) << 8 | buf.getByte((int)(offset + 2)) & 255;
    }

    static int getUnsignedInt(ByteBuf buf, int offset) {
        return (buf.getByte((int)offset) & 127) << 24 | (buf.getByte((int)(offset + 1)) & 255) << 16 | (buf.getByte((int)(offset + 2)) & 255) << 8 | buf.getByte((int)(offset + 3)) & 255;
    }

    static int getSignedInt(ByteBuf buf, int offset) {
        return (buf.getByte((int)offset) & 255) << 24 | (buf.getByte((int)(offset + 1)) & 255) << 16 | (buf.getByte((int)(offset + 2)) & 255) << 8 | buf.getByte((int)(offset + 3)) & 255;
    }
    ...
    static void validateHeaderName(CharSequence name) {
        ...
        if (name.length() == 0) {
            throw new IllegalArgumentException((String)"name cannot be length zero");
        }
        if (name.length() > 65535) {
            throw new IllegalArgumentException((String)("name exceeds allowable length: " + name));
        }
        int i = 0;
        while (i < name.length()) {
            char c = name.charAt((int)i);
            if (c == '\u0000') {
                throw new IllegalArgumentException((String)("name contains null character: " + name));
            }
            if (c >= 'A' && c <= 'Z') {
                throw new IllegalArgumentException((String)"name must be all lower case.");
            }
            if (c > '') {
                throw new IllegalArgumentException((String)("name contains non-ascii character: " + name));
            }
            ++i;
        }
    }
    ...
    static void validateHeaderValue(CharSequence value) {
        ...
        while (i < value.length()) {
            ...
            if (c == '\u0000') {
                throw new IllegalArgumentException((String)("value contains null character: " + value));
            }
            ++i;
        }
    }
