...
extends ReplayingDecoder<Void> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        if (this.discardingTooLongFrame) {
            buffer.skipBytes((int)this.actualReadableBytes());
            this.checkpoint();
            return;
        }
        Unmarshaller unmarshaller = this.provider.getUnmarshaller((ChannelHandlerContext)ctx);
        Object input = new ChannelBufferByteInput((ByteBuf)buffer);
        if (this.maxObjectSize != Integer.MAX_VALUE) {
            input = new LimitingByteInput((ByteInput)input, (long)((long)this.maxObjectSize));
        }
        try {
    ...
    protected void decodeLast(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        switch (buffer.readableBytes()) {
            ...
            case 1: {
                if (buffer.getByte((int)buffer.readerIndex()) != 121) break;
                buffer.skipBytes((int)1);
                return;
            }
        }
        this.decode((ChannelHandlerContext)ctx, (ByteBuf)buffer, out);
    }
