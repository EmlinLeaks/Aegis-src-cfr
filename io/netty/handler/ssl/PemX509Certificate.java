...
implements PemEncoded {
    ...
    static PemEncoded toPEM(ByteBufAllocator allocator, boolean useDirect, X509Certificate ... chain) throws CertificateEncodingException {
        ...
        if (chain == null) throw new IllegalArgumentException((String)"X.509 certificate chain can't be null or empty");
        if (chain.length == 0) {
            throw new IllegalArgumentException((String)"X.509 certificate chain can't be null or empty");
        }
        ...
        try {
            for (X509Certificate cert : chain) {
                if (cert == null) {
                    throw new IllegalArgumentException((String)("Null element in chain: " + Arrays.toString((Object[])chain)));
                }
                pem = cert instanceof PemEncoded ? PemX509Certificate.append((ByteBufAllocator)allocator, (boolean)useDirect, (PemEncoded)((PemEncoded)((Object)cert)), (int)chain.length, (ByteBuf)pem) : PemX509Certificate.append((ByteBufAllocator)allocator, (boolean)useDirect, (X509Certificate)cert, (int)chain.length, (ByteBuf)pem);
            }
            PemValue value = new PemValue((ByteBuf)pem, (boolean)false);
            success = true;
    ...
    private static ByteBuf append(ByteBufAllocator allocator, boolean useDirect, PemEncoded encoded, int count, ByteBuf pem) {
        ...
        if (pem == null) {
            pem = PemX509Certificate.newBuffer((ByteBufAllocator)allocator, (boolean)useDirect, (int)(content.readableBytes() * count));
        }
        pem.writeBytes((ByteBuf)content.slice());
        return pem;
    }
    ...
    private static ByteBuf append(ByteBufAllocator allocator, boolean useDirect, X509Certificate cert, int count, ByteBuf pem) throws CertificateEncodingException {
        ...
        try {
            ByteBuf base64 = SslUtils.toBase64((ByteBufAllocator)allocator, (ByteBuf)encoded);
            try {
                if (pem == null) {
                    pem = PemX509Certificate.newBuffer((ByteBufAllocator)allocator, (boolean)useDirect, (int)((BEGIN_CERT.length + base64.readableBytes() + END_CERT.length) * count));
                }
                pem.writeBytes((byte[])BEGIN_CERT);
                pem.writeBytes((ByteBuf)base64);
                pem.writeBytes((byte[])END_CERT);
                return pem;
            }
            finally {
                base64.release();
    ...
    private static ByteBuf newBuffer(ByteBufAllocator allocator, boolean useDirect, int initialCapacity) {
        ByteBuf byteBuf;
        if (useDirect) {
            byteBuf = allocator.directBuffer((int)initialCapacity);
            return byteBuf;
        }
        byteBuf = allocator.buffer((int)initialCapacity);
        return byteBuf;
    }

    public static PemX509Certificate valueOf(byte[] key) {
        return PemX509Certificate.valueOf((ByteBuf)Unpooled.wrappedBuffer((byte[])key));
    }

    public static PemX509Certificate valueOf(ByteBuf key) {
        return new PemX509Certificate((ByteBuf)key);
    }
    ...
    public PemX509Certificate replace(ByteBuf content) {
        return new PemX509Certificate((ByteBuf)content);
    }
