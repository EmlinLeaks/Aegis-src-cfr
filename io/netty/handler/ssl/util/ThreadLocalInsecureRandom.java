...
extends SecureRandom {
    ...
    public void nextBytes(byte[] bytes) {
        ThreadLocalInsecureRandom.random().nextBytes((byte[])bytes);
    }

    @Override
    public byte[] generateSeed(int numBytes) {
        byte[] seed = new byte[numBytes];
        ThreadLocalInsecureRandom.random().nextBytes((byte[])seed);
        return seed;
    }
    ...
    public int nextInt(int n) {
        return ThreadLocalInsecureRandom.random().nextInt((int)n);
    }
