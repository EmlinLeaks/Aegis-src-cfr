...
implements RowData {
    ...
    public boolean isAfterLast() {
        if (!this.lastRowFetched) return false;
        if (this.currentPositionInFetchedRows <= this.fetchedRows.size()) return false;
        return true;
    }
    ...
    public boolean isBeforeFirst() throws SQLException {
        if (this.currentPositionInEntireResult >= 0) return false;
        return true;
    }
    ...
    public boolean isEmpty() throws SQLException {
        if (!this.isBeforeFirst()) return false;
        if (!this.isAfterLast()) return false;
        return true;
    }
    ...
    public boolean isLast() throws SQLException {
        if (!this.lastRowFetched) return false;
        if (this.currentPositionInFetchedRows != this.fetchedRows.size() - 1) return false;
        return true;
    }
    ...
    public boolean hasNext() throws SQLException {
        ...
        if (this.currentPositionInEntireResult != -1) {
            ...
            this.fetchMoreRows();
            if (this.fetchedRows.size() <= 0) return false;
            return true;
        }
        this.fetchMoreRows();
        if (this.fetchedRows.size() <= 0) return false;
    ...
    public ResultSetRow next() throws SQLException {
        if (this.fetchedRows == null && this.currentPositionInEntireResult != -1) {
            throw SQLError.createSQLException((String)Messages.getString((String)"ResultSet.Operation_not_allowed_after_ResultSet_closed_144"), (String)"S1000", (ExceptionInterceptor)this.mysql.getExceptionInterceptor());
        }
        if (!this.hasNext()) {
            return null;
        }
        ...
        ResultSetRow row = this.fetchedRows.get((int)this.currentPositionInFetchedRows);
        row.setMetadata((Field[])this.metadata);
        return row;
    }
    ...
    private void fetchMoreRows() throws SQLException {
        ...
        if (this.lastRowFetched) {
            this.fetchedRows = new ArrayList<ResultSetRow>((int)0);
            return;
        }
        ...
        this.fetchedRows = this.mysql.fetchRowsViaCursor(this.fetchedRows, (long)this.statementIdOnServer, (Field[])this.metadata, (int)numRowsToFetch, (boolean)this.useBufferRowExplicit);
